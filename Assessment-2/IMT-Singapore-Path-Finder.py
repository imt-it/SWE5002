...
class CL_GraphManager:
    ...
    def __init__(self):
        ...
        self.boarding_delay_sec = 30  # Boarding delay at each station in seconds
            ...
            # Else if we use realistic time durations (simulated by real-world average train speeds and calculated distances)
                ...
                if conn_type == 'Internal_Connection_Branch':
                    ...
                    if from_station.train_type == 'MRT':
                        weight = (distance_m / self.speed_meters_per_sec_mrt_train) + self.boarding_delay_sec
                    else:
                        weight = (distance_m / self.speed_meters_per_sec_lrt_train) + self.boarding_delay_sec
                weight_str = f"{weight:.2f} sec"
        else:
    ...
    def list_stations_by_line(self, selected_line):
        FN_Print(f"\n[INFO]: Stations for Line {selected_line}:", 0)
        ...
        if line_info:
            ...
            FN_Print(f"[INFO]: No stations found for the selected line.", 0)
...
def main():
    ...
    graph_manager.load_connections(connection_csv, mode='unweighted') # Load the connections into the graph in unweighted mode (to have the list of stations available)

    # Primary Execution Thread
    while True:
        # Display the main menu options
        FN_Print(f"\n[INFO]: Please select one of the following options.", 0)
        ...
        if choice in ['1', '2', '3']:
            ...
            if choice == '3':
                # If the user selected the fastest path, ask if they want to use realistic time durations
                while True:
            ...
            start = input("\n[INPUT]: Enter start station code (leave blank for random): ").strip()
            end = input("[INPUT]: Enter end station code (leave blank for random): ").strip()
            ...
            # Calculate total weight (distance or time)
                if choice in ['2', '3']: # Only relevant for distance or time modes
                    total_weight = 0
                    for i in range(len(path) - 1):
                        edge_data = graph_manager.graph.get_edge_data(path[i], path[i + 1])
                        if edge_data and 'weight' in edge_data:
                            total_weight += edge_data['weight']
                    if choice == '2':
                        FN_Print(f"Total Distance: {total_weight:.2f} meters", 0)
                    elif choice == '3':
                        FN_Print(f"Total Time: {total_weight:.2f} seconds", 0)
            else:
        ...
        # List all stations by line
            ...
            print("\n[INFO]: Available Lines:")
            for i, line in enumerate(lines, 1):
                FN_Print(f"{i}. {line}", 0)
            # Prompt the user to select a line
            line_choice = input("[INPUT]: Enter the number of the line to view stations: ")
            try:
                ...
                if 1 <= line_choice <= len(lines):
                    ...
                    FN_Print(f"[INPUT]: Invalid line number. Please try again.", 0)
            except ValueError:
                FN_Print(f"[INPUT]: Invalid input. Please enter a number.", 0)
